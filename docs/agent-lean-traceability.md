...
| superfici linguistiche strutturate R5.37 | **parziale, con due task kind model-mediated concretamente esercitati** | Responsibility/LocalGoal compiler restano deterministici nel boundary 0029/0030 e non dipendono dal runtime modello. Il provider-neutral runtime 0031 esercita E2E `answerFromAuthorizedContext` e `interpretProxyRequest` con schema chiuso, ID grounded e retry bounded; explicit failure è persistito senza artifact autorevole. `summarizeGovernanceDecision` ha tipo/boundary validabile ma non un E2E production abilitato; `rewritePrompt` e gli altri adapter linguistici restano fail-closed. Nessun provider esterno è integrato. |
| no placeholder/provider permissivo | **completo** | I path senza adapter falliscono chiusi. |
| no model memory | **concretamente raffinato per le invocation 0031; external provider assumption residua** | Sprout non persiste memoria cognitiva, session ID provider, embedding o cache semantica parallela. Ogni invocation ricostruisce source correnti; dispatch, signed actual observation e projection devono coincidere e `hidden_persistent_model_memory_available=true` è rifiutato. Questo non attesta che un futuro provider non conservi dati internamente e non copre superfici ancora disabilitate. |
