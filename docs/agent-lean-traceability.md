...
| requirements/GoalContract compilation bounded | **coperto per il compiler locale endpoint-TCB** | `PromptRequirement`, binding bidirezionali requirement/obligation/WorkSpec, security policy per WorkSpec, exact action/tool policy e massimi server-side sono validati prima della persistenza. `retryTool` resta staticamente fail-closed quando manca la provenance dell'originale ToolCall; l'exact retry binding resta un obbligo runtime. |
...
| stessa governance alla creazione iniziale agent | **coperto per Responsibility e ApprovedAdministratorAgentCreation** | Test API completi esercitano user/controller con active compiled Responsibility e administrator creator con exact signed approval. `ApprovedLocalGoalException` e `GlobalMandateAssignment` restano fail-closed perché non hanno adapter autorevoli completi; non esiste più un quarto ramo generico `ProjectAdministrator`. |
...
| source-grounded global synthesis | **parziale, superficie esistente preservata** | Candidate strutturato da administrator client o runner autorizzato, source attive con Responsibility corrente e grounding non-amplifying sono verificati. I gate positivi coprono entrambi i boundary; una LocalGoal con origine `administratorCreation` è rifiutata come source bottom-up. GlobalMandate materialization e la projection totale R5.40/R5.41 restano aperti. |
...
| superfici linguistiche strutturate R5.37 | **parziale, con compiler governance concretamente abilitati** | Responsibility e LocalGoal dispongono di envelope/output specifici schema-closed, certificate persistiti e dual signature da endpoint TCB. Il server non integra né simula un provider LLM. Global synthesis strutturata preesistente resta raggiungibile da admin client/runner per source Responsibility-grounded; TaskIntent resta deterministico nel workflow cross-owner. Summaries e interrogation answer non hanno adapter; UserProxy planning resta parziale. |
...
| operational histories append-only | **parziale forte complessiva; coperto per TaskIntent/provenance e ledger governance 0029** | Audit agent/user-governance, compilation certificate, exact final approval, administrator creation approval e revision ledger sono append-only; app role non può INSERT verified né UPDATE/DELETE history e le writer `SECURITY DEFINER` hanno owner/search-path/privilegi fissati. Replay concorrente identico converge sulla stessa entry/position, equivocation confligge senza suffisso residuo. Per TaskIntent/provenance il ledger immutabile e `semantic_task_intent_list`/`semantic_task_provenance_list` definiscono una lista ordinata prefix-append-only attraverso retention. Transcript/message history completa e gli altri materializer R4 restano gap. |
...
| `R541ContractSecurityPolicyCertificate` | **concretamente raffinato per action staticamente risolvibili** | Ogni WorkSpec ha policy con ID esatto; resource operation e tool nominato devono coincidere ed essere nell'envelope server-side. `retryTool` senza original ToolCall provenance resta fail-closed. |
...
| `OperationalLocalRevisionActivationCertificate` | **concretamente raffinato per revisioni via Responsibility** | Current Responsibility, permission, membership, compiler build e signing keys vengono rivalidati nella transaction; stale/superseded activation rollbacka state e audit. Exception/global mandate restano fail-closed. |
