...
| exact final prompt approval | **parziale forte per activation manuale** | Il nuovo path lega draft, agent, controller, ciphertext hash, LocalGoal id/revision e timestamp; activation, system prompt, certificato e audit sono atomici e i mismatch testati falliscono. Non è copertura globale di `ControllerFinalPromptApproval`: il server non confronta i byte plaintext del prompt E2EE e il backfill 0027→0028 deriva certificati da righe già `active` senza un witness storico indipendente dell'approvazione finale. |
...
| operational histories append-only | **parziale forte complessiva; coperto per TaskIntent/provenance** | Audit agent/user-governance, final-prompt approval ed effect certificate sono append-only. Per TaskIntent/provenance il ledger immutabile e `semantic_task_intent_list`/`semantic_task_provenance_list` definiscono una lista ordinata prefix-append-only: il purge autenticato trasferisce solo il record live senza cambiarne posizione/contenuto, append successivi vanno in coda e replay/restart non duplicano. GUC forgiato e spoofing semantic-list da role `NOSUPERUSER/NOBYPASSRLS` falliscono. Transcript/message history completa e gli altri materializer R4 restano gap. |
