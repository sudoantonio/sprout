...
suite DB-enabled `agent_completion_gates` (cinque test, esecuzione seriale):
...
Il quinto gate esercita il materializer task generico con provenance
`LocalGoal/obligation/WorkSpec → canonical WorkItem → claim/attempt → task
completion → outcome → mechanical evidence`. `TaskIntent` non è una
...
| evidence meccanica/semantica e provenance | **parziale forte** | Evidence è schema-closed e derivata dal server. Per `TaskCompleted` il materializer crea il binding generico autorevole run/work/claim/attempt/task e l'outcome validato è l'unica fonte di mechanical evidence; stesso agent/scope/tempo e un task ID scelto dal runner non bastano. La projection temporale SQL usa lo stesso secondo contenitore (`floor(epoch)`) di `DateTime::timestamp()`. Gli adapter mechanical non-task e semantic judgment restano fail-closed/boundary. |
| discharge e accepted-evidence closure | **coperto per TaskCompleted mechanical** | Il kernel discharge soltanto tramite `accept_evidence`; il certificato DB richiede outcome causale, rule/mechanical mode, subject WorkSpec esatto e snapshot con obligation discharged. Blocker terminality non entra in questo percorso. Altri evidence kind restano parziali/fail-closed. |
...
| current permission/revocation | **completo per invocation/Info/task completion/cross-owner assignment** | Permission e runner/device key correnti sono rivalidati nella transaction dell'effect. Il materializer cross-owner richiede l'esatto `manage/full` dalla projection canonica (non creator metadata), e una revoca fra `ready` ed effect fallisce chiusa. Va esteso ai futuri effect/tool. |
...
| exact final prompt approval | **coperto per activation manuale** | Ogni draft prompt riceve un `draft_id` stabile. `agent_prompt_final_approvals` lega esattamente draft, agent, controller, ciphertext hash, LocalGoal id/revision e timestamp; un constraint trigger differito vieta un prompt `active` senza certificato corrispondente. Activation, update dell'esatto system prompt, certificato e audit committano atomicamente; draft/hash forgiati e stale retry falliscono senza record parziali. |
...
| TaskIntent e obligation provenance persistiti | **coperto per i path implementati** | `TaskObligationProvenance` ha identità autonoma e collega task→agent→LocalGoal revision→obligation→WorkSpec. `TaskIntent` è opzionale nel bridge generico e obbligatorio nel governance contract cross-owner. Facts/route sono server-side; live+retained projection preserva append-only logico dopo purge autenticato. |
...
| operational histories append-only | **parziale forte** | Audit agent/user-governance, final-prompt approval, TaskIntent, obligation provenance ed effect certificate sono append-only. Il purge autenticato archivia prima di eliminare il live row e le projection `semantic_task_intent_projection`/`semantic_task_obligation_provenance_projection` dimostrano continuità logica live+retained; GUC forgiato da role NOSUPERUSER/NOBYPASSRLS fallisce. Transcript/message history completa e gli altri materializer R4 restano gap. |
