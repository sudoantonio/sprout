...
## Projection concreta del checkpoint 0034

| Cluster formale | Stato concrete del checkpoint | Limite esplicito |
| --- | --- | --- |
| `R540ConcreteExecutionTrace.id` | **CONCRETELY REFINED per run 0034-native** | La Lean usa un `Nat` run-level; il DB usa il server-owned `trace_number bigint > 0`, creato soltanto insieme alla transition `initialized` con semantic tick esatto. Run 0033/legacy senza tale initialization restano prive di root e fail-closed. |
| `R540WorkAttemptEventExact` | **CONCRETELY REFINED per attempt external-tool 0034-native** | Una transition append-only `tool_attempt_opened` osserva, ma non aggiunge, un SemanticEvent Lean. Snapshot work/claim, actor, attempt e tick sono exact-bound e la claim deve essere valida al `requestedAt`. |
| `R540ToolEventExact` | **PARTIAL / EXTERNAL TCB al boundary typed-value→commitment; CONCRETELY REFINED per coordinate e commitment** | Pending e terminal tool records sono per-attempt immutabili e condividono run/goal/work/claim/attempt/actor/tick con il WorkAttempt. `V.ToolInput`/`V.ToolOutput` sono rappresentati da commitment tipizzati firmati dall'edge; il backend plaintext-blind prova binding e collision-resistant commitments, non equivalenza semantica plaintext. |
| `R540WorkOutcomeEventExact` | **CONCRETELY REFINED per terminal tool attempts 0034-native** | Outcome e transition snapshot devono mostrare lo stesso attempt e stato terminale. Failure N non può usare la successiva transition eligible/rearmed; il re-arm N+1 resta separato. |
| ordered trace inventory/certificate | **CONCRETELY REFINED per il tool cluster** | Un unico ordinal immutabile e unique per trace alimenta liste ordinate distinte di WorkAttempt, ToolEvent e WorkOutcome. Ogni certificate version append-only impegna le liste complete e il precedente certificate; count equality non certifica nulla. |
| tool/outcome `R541SurfaceGate` | **CONCRETELY REFINED per liste tool-cluster certificate** | `enabled` iff la lista exact certificata è non vuota; `disabledFailClosed` iff è vuota. Corruzione, mismatch, history incompleta o legacy rimuovono records e disabilitano il gate. Non è un full `R541TraceFeatureGateCertificate`: blocker/causal/evidence/disclosure e le altre cluster trace non sono incluse. |
| ToolOutput producer/consumer relation | **CONCRETELY REFINED per owner-only v1; PARTIAL per trace causale consumer** | `call.owner = effect.actor`; il reader è verificato separatamente tramite trusted `outputReadableBy`, e reader device/envelope e runner dispatch sono distinti. Producer WorkAttempt e consumer WorkBinding non devono coincidere. La relazione fra producer trace e una successiva model-consumer trace non viene collassata o promossa a full R540 certificate. |
| legacy upgrade | **FAIL-CLOSED** | 0034 non backfilla trace number, semantic tick, event, inventory o certificate. Dati operativi 0033 restano leggibili ma non entrano nelle surface. |
